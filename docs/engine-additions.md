...
## 8. Code review round 2 (`docs/code-review-2.md`)

Covered by `tests/engine-review2-engine.test.js` and `tests/engine-review2-zip.test.js`.

- **DOCX import markers** (`runsToInline`): whitespace at a run edge is emitted outside the markers — `Hello` + bold ` World` → `Hello **World**`, bold `bold ` + italic `italic` → `**bold** *italic*`. An all-whitespace run changes no formatting. (The old `{[X]}** rest**` expectation in `engine-fix2-docx-fields.test.js` was the bug and now reads `{[X]} **rest**`.)
- **`@pattern` is user regex on user input**: `patternProblem()` / `compilePattern()` in `engine/analyze.js` cap the source at 256 characters, refuse the catastrophic-backtracking shape (a quantified group that ends in a quantified atom or repeats an alternation with quantifiers: `(a+)+`, `(\d*)*`, `(x+){2,}`, `(a+|b)*`; `(\d+\.)*`, `(-\d{4})?`, `(\d{1,3}\.){3}` are fine), and memoise the compiled RegExp (512 entries). `collectAnnotations` reports a refused pattern as an `@pattern` error; `validate()` reports `X: invalid pattern /…/` (same text as a syntax error) and only ever tests the first 4096 characters of the value.
- **Zip bombs**: `readZip` now also enforces a package budget (`MAX_PACKAGE_SIZE`, 256 MB summed `uncompressedSize`, never below `maxEntrySize`) and an entry-count cap (`MAX_ENTRIES`, 4096). Entries expose `raw()` (stored bytes). `fillDocx` inflates only template parts that carry tags; every other entry (images, fonts, embedded files, tag-free headers) passes through as `{name, raw, method, crc, compressedSize, uncompressedSize}` — `writeZipAsync` / `writeZip` copy those verbatim (method, crc and sizes preserved, never re-encoded; entries with a method zipread cannot inflate, e.g. bzip2, survive the round trip too).
- **Options as `{value, label}` objects** are honoured by multiselect validation and `@default` (case-insensitive value match, like selection).
- **`escapeXml`** strips U+FFFE / U+FFFF and replaces lone surrogates with U+FFFD.
- **`@validate`** uses the template's `truthy()` (a whitespace-only string, `0`, `[]` fail the rule), the same truthiness as `{[if …]}`.
- **`Infinity`** is not a number: `coerce` leaves `"Infinity"` as text, `validate` reports `must be a number` for `±Infinity`, and `@default Infinity` is rejected.
- **Reserved path segments** (`constructor`, `prototype`, `__proto__`) in a template are a `TemplateError` (`"Client.constructor" is not a valid variable name (constructor is reserved)`) returned in `compile().errors` with the tag's line/col — no unanswerable variable is created.
- **List before item field**: `analyze()` registers a list before its item fields even when a concrete-index reference (`{[Trusts[0].Name]}`) precedes the `{[list Trusts]}` block, so `model.order` / `questionnaire()` ask for the list first. Object parents keep following their first field, as before.
