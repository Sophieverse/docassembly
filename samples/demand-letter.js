...
On {[IncidentDate|format:"long"]}, at {[IncidentLocation]}, {[if OnBehalfOfClient]}{[ClientName]}{[else]}I{[end if]} sustained injuries when {[IncidentDescription]} The incident was caused by {[if OnBehalfOfClient]}{[else]}your{[end if]}{[if OnBehalfOfClient]}{[Recipient.Name]}'s{[end if]} negligence, including {[NegligenceDescription]}. Injuries sustained include {[InjuryDescription]}, requiring medical treatment{[if TreatmentOngoing]} that is ongoing{[end if]}.
{[else]}
...
{[else if ClaimType = "Breach of Contract"]}
{[if OnBehalfOfClient]}{[ClientName]}{[else]}I{[end if]} {[if OnBehalfOfClient]}is{[else]}am{[end if]} entitled to recover all damages that are the natural and proximate result of the breach, including consequential damages within the parties' contemplation at contracting. {[if Statute.Citation]}See {[Statute.Citation]}. {[end if]}{[if Statute.FeesAvailable]}The Agreement provides for recovery of attorney's fees by the prevailing party.{[end if]}
{[else if ClaimType = "Personal Injury"]}
Under {[Statute.Citation|default:"applicable state negligence law"]}, a party who breaches a duty of care is liable for the resulting medical expenses, lost earnings, and pain and suffering. {[if Statute.LimitationsYears]}The applicable limitations period is {[Statute.LimitationsYears|words]} ({[Statute.LimitationsYears]}) year{[if Statute.LimitationsYears != 1]}s{[end if]} from the date of injury.{[end if]}
{[end if]}
{[end if]}
