 ...
 * tax classification, capital table, transfer restrictions / ROFR.
 */
...
{[if count(Members) = 1]}
**2.1 Sole Member.** {[list Members]}{[FullName]}, of {[Address]}, is the sole Member of the Company and holds one hundred percent (100%) of the membership interests. The Member has contributed {[Contribution|currency]} to the capital of the Company.{[end list]}
{[else]}
...
**5.1 Manager-Managed.** The Company is manager-managed. The business and affairs of the Company shall be managed exclusively by {[if count(Managers) = 1]}one Manager{[else]}{[count(Managers)|words]} Managers acting by majority{[end if]} (the "Manager"). The initial Manager{[if count(Managers) > 1]}s are{[else]} is{[end if]}: {[list Managers]}{[FullName]}{[_punc]}{[end list]}
...
{[else]}
**6.1 Transfers.** A Member may Transfer all or any part of its membership interest upon written notice to the other Members. A transferee shall be admitted as a Member upon signing a joinder to this Agreement.
{[end if]}
...
**7.3 Payment.** The purchase price shall be paid {[if BuyoutInstallmentYears > 0]}in equal monthly installments over {[BuyoutInstallmentYears|words]} ({[BuyoutInstallmentYears]}) years, with interest at the applicable federal rate, evidenced by a promissory note{[else]}in cash at closing{[end if]}.
{[end if]}
...
{[FullName]}
{[if count(Members) > 1]}Percentage Interest: {[Percent]}%{[end if]}
{[end list]}
{[if ManagementType = "Manager-managed"]}
{[list Managers]}
...
export const sampleAnswers = {
  ...
  Members: [
    { FullName: 'Aisha Rahman', Address: '14 Elm Court, Wilmington, DE 19806', Contribution: 60000, Percent: 60 },
    { FullName: 'Marcus Delgado', Address: '88 Harbor View, Philadelphia, PA 19106', Contribution: 30000, Percent: 30 },
    { FullName: 'Yuki Tanaka', Address: '502 Pine Street, Newark, DE 19711', Contribution: 10000, Percent: 10 },
  ],
